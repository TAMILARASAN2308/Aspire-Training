...
router.get('/ticket/:id', (req, res) => {
    ...
    const query = `
        SELECT f.ID, f.FROMLOCATION, f.TOLOCATION, f.DEPARTUREDATE, f.RETURNDATE, f.CLASS, 
               p.FULLNAME, p.GENDER, p.EMAIL, p.PHONENUMBER
        FROM flightinfo f 
...
router.get('/search', (req, res) => {
    ...
    db.query(checkQuery, queryParams, (error, results) => {
        ...
        if (results.length > 0) {
            ...
            const insertQuery = `
                INSERT INTO flightinfo (FROMLOCATION, TOLOCATION, DEPARTUREDATE, RETURNDATE, CLASS)
                VALUES (?, ?, ?, ?, ?);
            `;

            const insertParams = [fromLocation, toLocation, departureDate, returnDate, classType];
